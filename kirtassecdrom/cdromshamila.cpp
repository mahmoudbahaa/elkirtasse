/****************************************************************************
//   elkirtasse Copyright (C) 2011 yahia nouah <yahiaui@gmail.com>
//
...
bool cdromShamila::creatBooksInfo()
{
    ...
    for (int i=1;i<tb+2;i++){
        ...
        if( creatBook(bkCat+"_"+bkId)==true){
            progress.setLabelText(trUtf8("الرجاء الانتظار..جاري انشاء :")+bkName);

            addNewBook(bkId,bkName,bkAut,bkBetaka,bkCat);
            saveBookInfo("bk"+ bkId,bkName,bkAut,bkBetaka);
...
bool cdromShamila::creatBook(QString id){
  listId.clear();
    QFile file;
    QProcess prosses;
    ...
    if(file.exists(fileMdb)){
        ...
        prosses.waitForFinished();
        QDir dir;
        QString newBookPath=m_pathBooksNew+"/bk"+id;
